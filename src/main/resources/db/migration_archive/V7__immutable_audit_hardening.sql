...
-- 0. Ensure tenants table has latest status fields
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS subscription_status VARCHAR(32) NOT NULL DEFAULT 'SUBSCRIPTION_ACTIVE';
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS government_status VARCHAR(32) NOT NULL DEFAULT 'GOVERNMENT_ACTIVE';
...
ALTER TABLE audit_events ADD COLUMN IF NOT EXISTS payload_json TEXT;
...
-- 6. Receipts Table (MoR Official Receipts Entity)
...
CREATE INDEX IF NOT EXISTS idx_receipts_tenant_inv ON receipts(tenant_id, invoice_id);
CREATE INDEX IF NOT EXISTS idx_receipts_rrn ON receipts(rrn);
...
-- 7. Enforce Append-Only Immutability on audit_events
